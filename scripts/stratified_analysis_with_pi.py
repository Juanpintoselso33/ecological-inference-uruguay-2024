"""
Stratified analysis by region (Metropolitana vs Interior) WITH PI separate.
"""
...
def analyze_stratum(stratum_name, df_stratum, samples=1000, chains=2):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Stratified analysis by region (with PI separate)'
    )
    parser.add_argument(
        '--data',
        type=str,
    ...
    parser.add_argument(
        '--output-csv',
        type=str,
        default='outputs/tables/region_with_pi.csv',
    ...
    print("="*80)
    print("STRATIFIED ANALYSIS BY REGION - WITH PI SEPARATE")
    print("="*80)
    ...
    df['region'] = df['departamento'].apply(
        lambda x: 'Metropolitana' if x in ['Montevideo', 'Canelones'] else 'Interior'
    )
    ...
    # Print comparison
    print("\n" + "="*80)
    print("COMPARISON: METROPOLITANA VS INTERIOR")
    print("="*80)
