"""
Analyze coalition transfers by department - INCLUDING PI as separate party.
"""
...
def analyze_department(dept_name, df_dept, quick=False):
    ...
    results = {
        ...
        'pi_votes': df_dept['pi_primera'].sum(),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Analyze coalition transfers by department (with PI separate)'
    )
    parser.add_argument(
        '--data',
        type=str,
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='outputs/tables/transfers_by_department_with_pi.csv',
    ...
    print(f"PI votes (primera vuelta): {df['pi_primera'].sum():,.0f} ({df['pi_primera'].sum()/df['total_primera'].sum():.2%})\n")
    ...
    for i, dept in enumerate(departments, 1):
        ...
        if result:
            ...
            print(f"  CA to FA: {result['ca_to_fa']*100:.1f}%, "
                  ...
                  f"PI to FA: {result['pi_to_fa']*100:.1f}%, "
    ...
    # Summary statistics
    print("\n" + "="*70)
    print("SUMMARY BY DEPARTMENT (WITH PI SEPARATE)")
    print("="*70)
    ...
    print("\n" + "="*70)
    print("PI Defection to FA (highest to lowest):")
    print("-"*70)
    results_df_sorted = results_df.sort_values('pi_to_fa', ascending=False)
    for _, row in results_df_sorted.head(10).iterrows():
        print(f"{row['departamento']:20s}: {row['pi_to_fa']*100:5.1f}% "
              f"({row['pi_votes']:,.0f} votes)")
